...
/**
 * This class uses the Test4 class (so don't delete it!). It should solve the entire puzzle on it's own.
...
public class FormatTest4 {
	...
	public static void main(String[] args) {
		...
		System.out.println("Input Letters in the format it appears in. Use * for blank spots (don't use spaces ever):"); // Asks for user to input letters available.
		String letters = "";
		Vector<Vector<Character>> format = new Vector<Vector<Character>>(); 
		
		while (scan.hasNextLine()) {
			
			letters = scan.nextLine(); // Puts letters into a String called "letters".
			format.add(separateLetters(letters));
		...
		for (Vector<String> vs : words) {
			...
			for (String s : vsDupe) {
				...
				if (hasOneOfEveryLetter.test(separateLetters(s)) == true) {
					...
					/* TODO 
					 * 1. use a isViable method in PuzzleFormatManager on the word
					 * 2. Recreate the format
					 * 3. plug the format into PuzzleFormatManager and repeat step 1-2 until solved!
					 */
